...
while True:
    ...
    data = (packet['data'])
    storagex = 25 # team 1: 25; 2: 275
    storagey = 150 # team 1: 150; 2: 150
    listobject = []
    for i in range(len(data)):
        name = str(data[i]['name'])
        x = data[i]['position'][0]
        y = data[i]['position'][1]
        if name[0] == 'o' and not (storagex - 40 <= x <= storagex + 40 and storagey - 25 <= y <= storagey + 25):
            listobject.append([math.hypot(x - storagex, y - storagey), x, y])
        #Vis.draw(name,i,x,y)    #Vis.draw(name,i,x,y)
    listobject = sorted(listobject)

    auid = 20
    vec[0] = listobject[0][1]-data[auid]['position'][0]
    vec[1] = listobject[0][2]-data[auid]['position'][1]
    error = vAngle(data[auid]['dimension'],vec) 
    #print(error)
    if error < 5 and vec[0] * vec[0] + vec[1] * vec[1] < 25:
        vec[0] = storagex-data[auid]['position'][0]
        vec[1] = storagey-data[auid]['position'][1]
        error = vAngle(data[auid]['dimension'],vec)
    ...
    if storagex - 40 <= data[auid]['position'][0] <= storagex + 40:
        if storagey - 25 <= data[auid]['position'][1] <= storagey + 25:
            vec[0] = data[auid]['position'][0]
            vec[1] = 0
            time.sleep(0.5)